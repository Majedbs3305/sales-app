...
import { Plus, TrendingUp, Users, Package, Clock, Trash2, X, BarChart3, List, WifiOff, Lock, LogOut, Eye, EyeOff, BookOpen, ChevronDown, Hash, MessageSquare } from 'lucide-react';
...
export default function App() {
  ...
  return <SalesApp token={token} onLogout={handleLogout} />;
}
...
function LoginScreen({ onSuccess }) {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // نعرض رسالة الخطأ الحقيقية (فيها عداد المحاولات أو مدة القفل المتبقية)
    ...
    <div dir="rtl" style={styles.loginPage}>
      <style>{`
        ...
        * { box-sizing: border-box; font-family: 'IBM Plex Sans Arabic', sans-serif; }
        body { margin: 0; }
        input:focus { outline: 2px solid #2F6F5E; outline-offset: 1px; }
      `}</style>
...
function SalesApp({ token, onLogout }) {
    ...
    <div dir="rtl" style={styles.page}>
      <style>{`
        ...
        * { box-sizing: border-box; font-family: 'IBM Plex Sans Arabic', sans-serif; }
        ...
        ::placeholder { color: #9C9186; }
        input:focus, textarea:focus { outline: 2px solid #2F6F5E; outline-offset: 1px; }
        button:focus-visible { outline: 2px solid #2F6F5E; outline-offset: 2px; }
      ...
      <div style={styles.body}>
        {loading ? (
          ...
          <LogView entries={entries} onDelete={handleDelete} />
        ) : (
          <ReportView totals={totals} maxSellerAmount={maxSellerAmount} maxBuyerAmount={maxBuyerAmount} />
        )}
      </div>
  ...
  if (entries.length === 0) {
    return (
      <div style={styles.emptyState}>
        <Package size={32} color="#B5AA9C" strokeWidth={1.5} />
        ...
        <div key={e.id} className="row-enter" style={styles.entryCard}>
          <div style={styles.entryMain}>
            ...
            <div style={styles.entryMeta}>
              <Clock size={12} color="#B5AA9C" />
              <span>{formatDateTime(e.time)}</span>
            </div>
            {e.notes && (
              <div style={styles.entryNotes}>
                <MessageSquare size={11} color="#B5AA9C" />
                <span>{e.notes}</span>
              </div>
            )}
          </div>
...
function ReportView({ totals, maxSellerAmount, maxBuyerAmount }) {
  if (totals.sellerList.length === 0) {
    return (
      <div style={styles.emptyState}>
        <BarChart3 size={32} color="#B5AA9C" strokeWidth={1.5} />
  ...
  return (
    <div style={styles.reportWrap}>
      <ReportSection icon={<TrendingUp size={16} color="#2F6F5E" />} title="حسب البائع" list={totals.sellerList} max={maxSellerAmount} color="#2F6F5E" />
      <ReportSection icon={<Users size={16} color="#B5622E" />} title="حسب المشتري" list={totals.buyerList} max={maxBuyerAmount} color="#B5622E" />
    </div>
  );
}

function ReportSection({ icon, title, list, max, color }) {
  return (
...
const styles = {
  loginPage: { minHeight: '100vh', background: '#211D19', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 },
  loginCard: { background: '#FBF9F5', borderRadius: 20, padding: '32px 28px', width: '100%', maxWidth: 360, textAlign: 'center' },
  loginIcon: { width: 48, height: 48, background: '#E7F2ED', borderRadius: 14, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px' },
  loginTitle: { fontSize: 22, fontWeight: 700, margin: '0 0 6px' },
  loginSub: { fontSize: 13, color: '#9C9186', margin: '0 0 20px' },
  loginInput: { width: '100%', padding: '13px 44px 13px 14px', borderRadius: 12, border: '1px solid #E3DCD0', fontSize: 15, background: '#FFFFFF', color: '#2B2621', textAlign: 'center' },
  passwordWrap: { position: 'relative', marginBottom: 12 },
  eyeBtn: { position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)', background: 'transparent', border: 'none', color: '#9C9186', cursor: 'pointer', padding: 4, display: 'flex', alignItems: 'center' },

  page: { minHeight: '100vh', background: '#FBF9F5', color: '#2B2621', paddingBottom: 40 },
  offlineBanner: { display: 'flex', alignItems: 'center', gap: 6, justifyContent: 'center', background: '#B5622E', color: '#fff', fontSize: 12, padding: '6px 0' },
  header: { background: '#211D19', padding: '28px 20px 0', borderRadius: '0 0 24px 24px' },
  headerTop: { display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 },
  headerBtns: { display: 'flex', gap: 8 },
  notebookSwitcher: { display: 'flex', alignItems: 'center', gap: 5, background: 'transparent', border: 'none', color: '#8A9E93', fontSize: 12, letterSpacing: '0.02em', marginBottom: 4, cursor: 'pointer', padding: 0, maxWidth: '100%' },
  notebookSwitcherText: { overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 180 },
  eyebrow: { fontSize: 12, letterSpacing: '0.05em', color: '#8A9E93', marginBottom: 4, fontWeight: 500 },
  title: { fontSize: 26, fontWeight: 700, color: '#F5F1E9', margin: 0 },
  addBtn: { display: 'flex', alignItems: 'center', gap: 6, background: '#2F6F5E', color: '#F5F1E9', border: 'none', borderRadius: 12, padding: '10px 16px', fontSize: 14, fontWeight: 600, cursor: 'pointer', boxShadow: '0 2px 8px rgba(47,111,94,0.35)' },
  logoutBtn: { display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#2A251F', color: '#B5AA9C', border: 'none', borderRadius: 12, padding: '10px 12px', cursor: 'pointer' },
  statsRow: { display: 'flex', gap: 10, marginBottom: 20 },
  statCard: { flex: 1.4, background: '#2A251F', borderRadius: 14, padding: '14px 16px' },
  statCardSm: { flex: 1, background: '#2A251F', borderRadius: 14, padding: '14px 16px' },
  statLabel: { fontSize: 11, color: '#9C9186', marginBottom: 6 },
  statValueMain: { fontSize: 22, fontWeight: 600, color: '#8FD4B8' },
  statValue: { fontSize: 18, fontWeight: 600, color: '#F5F1E9' },
  tabs: { display: 'flex', gap: 4 },
  tab: { display: 'flex', alignItems: 'center', gap: 6, flex: 1, justifyContent: 'center', padding: '12px 0', background: 'transparent', border: 'none', borderBottom: '2px solid transparent', color: '#9C9186', fontSize: 13, fontWeight: 600, cursor: 'pointer' },
  tabActive: { color: '#F5F1E9', borderBottom: '2px solid #2F6F5E' },
  body: { padding: '18px 16px 0' },
  list: { display: 'flex', flexDirection: 'column', gap: 8 },
  entryCard: { display: 'flex', alignItems: 'flex-start', gap: 10, background: '#FFFFFF', border: '1px solid #EDE8DF', borderRadius: 14, padding: '12px 14px' },
  ...
  buyerName: { color: '#2B2621' },
  arrow: { color: '#B5AA9C', fontSize: 12 },
  sellerName: { color: '#5C6B62' },
  meshBadge: { fontSize: 11, fontWeight: 600, color: '#2F6F5E', background: '#E7F2ED', borderRadius: 6, padding: '2px 6px' },
  entryMeta: { display: 'flex', alignItems: 'center', gap: 4, fontSize: 11, color: '#9C9186', marginBottom: 4 },
  entryNotes: { display: 'flex', alignItems: 'flex-start', gap: 4, fontSize: 12, color: '#8A8072', background: '#F7F4EE', borderRadius: 8, padding: '5px 8px', marginTop: 2 },
  entryNums: { textAlign: 'left', flexShrink: 0 },
  entryQty: { fontSize: 11, color: '#9C9186', display: 'flex', alignItems: 'center', gap: 3, justifyContent: 'flex-end' },
  qtyBadge: { fontWeight: 600 },
  qtyUnit: { color: '#B5AA9C' },
  entryAmount: { fontSize: 15, fontWeight: 700, color: '#2F6F5E' },
  deleteBtn: { background: 'transparent', border: 'none', color: '#D6C9BC', cursor: 'pointer', padding: 4, flexShrink: 0 },
  ...
  emptyText: { fontSize: 15, fontWeight: 600, color: '#6B6255' },
  emptySub: { fontSize: 13, color: '#9C9186' },
  reportWrap: { display: 'flex', flexDirection: 'column', gap: 24 },
  reportSection: { background: '#FFFFFF', border: '1px solid #EDE8DF', borderRadius: 16, padding: 16 },
  reportSectionTitle: { display: 'flex', alignItems: 'center', gap: 6, fontSize: 14, fontWeight: 700, marginBottom: 14, color: '#2B2621' },
  ...
  reportAmount: { fontWeight: 700, color: '#2B2621' },
  barTrack: { height: 6, background: '#F1ECE3', borderRadius: 4, overflow: 'hidden' },
  barFill: { height: '100%', borderRadius: 4 },
  reportRowSub: { display: 'flex', justifyContent: 'space-between', marginTop: 5, fontSize: 11, color: '#9C9186' },
  modalBackdrop: { position: 'fixed', inset: 0, background: 'rgba(20,17,14,0.5)', display: 'flex', alignItems: 'flex-end', justifyContent: 'center', zIndex: 50 },
  modalCard: { background: '#FBF9F5', borderRadius: '20px 20px 0 0', padding: '20px 20px 24px', width: '100%', maxWidth: 480, maxHeight: '85vh', overflowY: 'auto' },
  modalHeader: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 },
  modalTitle: { fontSize: 17, fontWeight: 700, margin: 0 },
  closeBtn: { background: '#F1ECE3', border: 'none', borderRadius: 10, padding: 6, cursor: 'pointer', color: '#6B6255' },
  formGroup: { marginBottom: 14 },
  formRow: { display: 'flex', gap: 10 },
  label: { display: 'block', fontSize: 12, fontWeight: 600, color: '#6B6255', marginBottom: 6 },
  input: { width: '100%', padding: '11px 14px', borderRadius: 12, border: '1px solid #E3DCD0', fontSize: 15, background: '#FFFFFF', color: '#2B2621' },
  textarea: { width: '100%', padding: '11px 14px', borderRadius: 12, border: '1px solid #E3DCD0', fontSize: 14, background: '#FFFFFF', color: '#2B2621', resize: 'none', fontFamily: 'inherit' },
  errorText: { color: '#B5622E', fontSize: 13, marginBottom: 12 },
  totalPreview: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#E7F2ED', borderRadius: 12, padding: '12px 16px', marginBottom: 14, fontSize: 13, fontWeight: 600, color: '#2F6F5E' },
